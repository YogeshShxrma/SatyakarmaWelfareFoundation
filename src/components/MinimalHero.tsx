...
const MinimalHero: React.FC<MinimalHeroProps> = ({
  ...
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      // Only prevent default if we're in the expansion phase
      if (!isExpanded || (isExpanded && e.deltaY < 0)) {
        e.preventDefault();
      }
      ...
      if (e.deltaY > 0 && !isExpanded) {
        ...
        // Scrolling up - always allow reverse
        accumulatedScroll -= Math.abs(e.deltaY);
        accumulatedScroll = Math.max(0, accumulatedScroll);
        if (isExpanded && accumulatedScroll < scrollThreshold) {
          setIsExpanded(false);
        }
      }
      ...
      // Update expansion state and scroll lock
      if (progress >= 1 && !isExpanded) {
        setIsExpanded(true);
      } else if (progress === 0 && isExpanded) {
        setIsExpanded(false);
      }

      // Only lock scroll when not expanded and not reversing
      if (!isExpanded && progress < 1) {
    ...
    const handleTouchStart = (e: TouchEvent) => {
      ...
      const handleTouchMove = (moveEvent: TouchEvent) => {
        ...
        // Only prevent default if we're in the expansion phase
        if (!isExpanded || (isExpanded && deltaY < 0)) {
        ...
        if (deltaY > 0 && !isExpanded) {
          ...
          // Swiping down - always allow reverse
          accumulatedScroll -= Math.abs(deltaY) * 2;
          accumulatedScroll = Math.max(0, accumulatedScroll);
          if (isExpanded && accumulatedScroll < scrollThreshold) {
            setIsExpanded(false);
    ...
    // Initialize scroll lock only if not expanded
    if (!isExpanded) {
      document.body.style.overflow = 'hidden';
    }
