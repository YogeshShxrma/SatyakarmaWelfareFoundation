...
function FallingLeaf({
  ...
  return <motion.div style={{
    ...
    y: ["-80px", "105vh"],
    // from just above to far below
    x: [0, sway, -sway, sway / 2, 0],
    ...
    times: [0, 0.25, 0.7, 0.96, 1]
      ...
      <Leaf size={30 * scale} color={color} strokeWidth={1 + scale * 0.5} className="drop-shadow-[0_1px_4px_rgba(106,121,95,0.13)]" aria-label="" focusable="false" />
    </motion.div>;
}
function FallingLeavesBG() {
  ...
  return <div className="absolute inset-0 pointer-events-none w-full h-full">
      {Array.from({
      length: total
    }).map((_, idx) => <FallingLeaf key={idx} idx={idx} total={total} />)}
    </div>;
}
