...
-- Drop all existing policies with correct names from the schema
...
DROP POLICY IF EXISTS "Authenticated admins can manage media" ON public.media;
...
-- Add file upload security constraints (including existing 'image' type)
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        CHECK (file_type IN ('image', 'image/jpeg', 'image/png', 'image/gif', 'image/webp', 'video/mp4', 'video/webm'));
    END IF;
END
$$;
